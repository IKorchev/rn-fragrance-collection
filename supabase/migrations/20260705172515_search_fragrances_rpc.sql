-- Catalog search RPCs + fragrance_notes indexes

CREATE INDEX IF NOT EXISTS fragrance_notes_fragrance_id_note_idx
  ON fragrance_notes (fragrance_id, note);
CREATE INDEX IF NOT EXISTS fragrance_notes_note_idx
  ON fragrance_notes (note);
...
AS $$
  ...
  ORDER BY
    CASE WHEN p.term IS NOT NULL
         THEN GREATEST(similarity(f.name, p.term), similarity(f.brand, p.term))
    END DESC NULLS LAST,
...
RETURNS TABLE (brand text, fragrance_count bigint)
LANGUAGE sql STABLE SET search_path = public
AS $$
  SELECT f.brand, COUNT(*)::bigint FROM fragrances f
  GROUP BY f.brand ORDER BY COUNT(*) DESC, f.brand ASC
$$;

CREATE OR REPLACE FUNCTION list_notes(
  search_term text DEFAULT NULL, max_results int DEFAULT 50
)
RETURNS TABLE (note text, fragrance_count bigint)
LANGUAGE sql STABLE SET search_path = public
AS $$
  SELECT fn.note, COUNT(DISTINCT fn.fragrance_id)::bigint
  FROM fragrance_notes fn
  WHERE NULLIF(TRIM(search_term), '') IS NULL
     OR fn.note ILIKE '%' || TRIM(search_term) || '%'
  GROUP BY fn.note
  ORDER BY COUNT(DISTINCT fn.fragrance_id) DESC, fn.note ASC
