...
-- ---------------------------------------------------------------------------
-- Streak Saver (Pro perk, migration add_streak_saves). Lets a Pro user spend
-- one of a monthly allowance of "saves" on a missed day so
...
-- Opt-in public profiles (migration add_public_profiles). A row is a
-- client-pushed snapshot of the owner's client-computed gamification state
-- (src/lib/gamification is pure TS — reimplementing streak/badge/level math
-- in SQL would drift), plus identity fields and the profile-header photo
-- path. Private by default; flipping is_public is the only thing that
-- exposes a row to other users (see the SELECT policy). Snapshot fields are
-- self-reported by the owner's client (kept fresh by
-- src/lib/utils/use-profile-sync.ts), so they're display/leaderboard data,
-- not an integrity boundary — the CHECKs just bound garbage. Real wear data
-- for a profile comes from user_top_worn below.
